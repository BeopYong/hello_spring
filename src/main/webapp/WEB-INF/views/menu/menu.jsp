...
<%--
	url: "http://localhost:10000/springboot/menus" 직접 springboot서버로 연결 요청. 아래 오류 유발
	...
	SOP Same Origin Policy 동일 근우너 정책
		- 같은 origin에만 비동기요청을 보낼 수 있다.
	...
	- 응답헤더에 Access-Control-Allow-Origin 현재 Origin이 등록되어 있어야 한다.
 --%>
		...
		<div id="menu-container" class="text-center">
			...
			$(menuEnrollFrm).submit((e) => {
				...
				$.ajax({
					...
					method: "POST",
					data: jsonStr,
					contentType: "application/json; charset=utf-8", // json으로 넘겨줄때 명시해주기
					success(resp){
						console.log(resp);
						alert(resp.msg);
					},
