	...
	<table class="table">
    	...
    	<c:forEach items="${memoList}" var="memoList">
		    <tr>
		    	<td>${memoList.no}</td>
				<td>${memoList.memo}</td>
				<td><fmt:formatDate value="${memoList.regDate}" pattern="yyyy/MM/dd"/></td>
				<td>
					<button class="btn btn-outline-danger btn-memo-delete" data-no="${memoList.no}" type="button" >삭제</button>
				</td>	
	    	</tr>
    	</c:forEach>
	</table>
